...
public class SwiftNekotonFlutterPlugin: NSObject, FlutterPlugin {
    ...
    public func dummyMethodToEnforceBundling() {
        ...
        nt_generic_contract_subscribe(0, 0, 0, 0, 0, nil, 0, nil);
        ...
        nt_keystore_storage_key();

        nt_keystore_create(0, nil);

        nt_keystore_entries(0, nil);

        nt_keystore_add_key(0, nil, nil);

        nt_keystore_add_keys(0, nil, nil);
        ...
        nt_keystore_decrypt(0, nil, nil, nil);

        nt_keystore_sign(0, nil, nil, nil);

        nt_keystore_sign_data(0, nil, nil, nil);

        nt_keystore_sign_data_raw(0, nil, nil, nil);

        nt_transport_get_signature_id(0, nil, 0);

        nt_transport_get_network_id(0, nil, 0);
        ...
        nt_token_wallet_subscribe(0, 0, 0, nil, 0, nil, nil);
        ...
        nt_get_token_root_details(0, nil, 0, nil);

        nt_get_token_wallet_details(0, nil, 0, nil);

        nt_get_token_root_details_from_token_wallet(0, nil, 0, nil);

        nt_token_wallet_clone_ptr(nil);

        nt_token_wallet_free_ptr(nil);

        nt_ton_wallet_subscribe(0, 0, 0, 0, 0, nil, 0, 0, nil, nil);

        nt_ton_wallet_subscribe_by_address(0, 0, 0, 0, 0, nil, 0, nil);

        nt_ton_wallet_subscribe_by_existing(0, 0, 0, 0, 0, nil, 0, nil);
        ...
        nt_find_existing_wallets(0, nil, 0, nil, 0, nil);

        nt_get_existing_wallet_info(0, nil, 0, nil);

        nt_get_wallet_custodians(0, nil, 0, nil);
        ...
        nt_storage_create(nil);

        nt_storage_get(0, nil, nil);

        nt_storage_set(0, nil, nil, nil);
        ...
        nt_transport_get_contract_state(0, nil, 0, nil);

        nt_transport_get_full_contract_state(0, nil, 0, nil);

        nt_transport_get_accounts_by_code_hash(0, nil, 0, nil, 0, nil);

        nt_transport_get_transactions(0, nil, 0, nil, nil, 0);

        nt_transport_get_transaction(0, nil, 0, nil);
