...
pub unsafe extern "C" fn token_wallet_subscribe(
    ...
    rt.spawn(async move {
        ...
        let result =
            internal_token_wallet_subscribe(port, transport, owner, root_token_contract).await;
...
#[no_mangle]
pub unsafe extern "C" fn token_wallet_prepare_deploy(
    result_port: c_longlong,
    token_wallet: *mut c_void,
    ...
    rt.spawn(async move {
        ...
        let result = internal_token_wallet_prepare_deploy(
            &mut token_wallet,
            &mut ton_wallet,
            transport,
            expiration,
...
async fn internal_token_wallet_prepare_deploy(
    token_wallet: &mut TokenWallet,
...
pub unsafe extern "C" fn token_wallet_handle_block(
    ...
    rt.spawn(async move {
        ...
        let result = internal_token_wallet_handle_block(&mut token_wallet, transport, id).await;
        let result = match_result(result);
...
#[no_mangle]
pub unsafe extern "C" fn token_wallet_unsubscribe(
    result_port: c_longlong,
    token_wallet: *mut c_void,
) {
    let token_wallet = token_wallet as *mut MutexTokenWallet;
    let token_wallet = Arc::from_raw(token_wallet);
    ...
    rt.spawn(async move {
        ...
        let result = internal_token_wallet_unsubscribe(token_wallet).await;
        let result = match_result(result);
        send_to_result_port(result_port, result);
    });
}

async fn internal_token_wallet_unsubscribe(
    token_wallet: Option<TokenWallet>,
) -> Result<u64, NativeError> {
    match token_wallet {
        Some(_) => Ok(0),
