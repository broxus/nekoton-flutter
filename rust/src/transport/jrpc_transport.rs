...
use crate::{
    ...
    models::{HandleError, MatchResult, ToPtrAddress, ToStringFromPtr},
};

#[no_mangle]
pub unsafe extern "C" fn nt_jrpc_transport_create(endpoint: *mut c_char) -> *mut c_char {
    let endpoint = endpoint.to_string_from_ptr();

    fn internal_fn(endpoint: String) -> Result<serde_json::Value, String> {
